...
What it does:
  ...
  * IDs are kept exactly as mycolog had them, so labels already written
    on your jars and plates stay correct. Only new entries you make in
    Sporeline get the new SE0K7P style IDs.
...
def convert(folder, out_path):
    ...
    for old_id, ctype, species, token, created, notes, gone in rows:
        ...
        g = gen.get(old_id, 0)
        # Keep mycolog's token as the ID so existing labels stay valid.
        # Only fall back to a generated ID if a token is missing or was
        # somehow issued twice.
        new = (token or "").strip().upper()
