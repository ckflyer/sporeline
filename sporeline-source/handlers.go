...
func handleList(w http.ResponseWriter, r *http.Request) {
	...
	render(w, "list.html", page{Title: k.Plural, Tab: kind, Data: listData{
		...
		AllSpecies: store.Seen("species"), Counts: counts, Total: len(items),
	}})
}
...
func handleRecipeForm(w http.ResponseWriter, r *http.Request) {
	...
	if rec == nil {
		rec = &Recipe{Category: "Agar", Ingredients: []Ingredient{{}, {}, {}}}
	}
	for len(rec.Ingredients) < 8 {
...
func handleRecipeSave(w http.ResponseWriter, r *http.Request) {
	...
	rec := &Recipe{
		...
		Sterilize: strings.TrimSpace(r.FormValue("sterilize")),
